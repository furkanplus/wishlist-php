...
if (file_exists($installLock) || file_exists($configFile)) {
    // Attempt connection to see if it is functional
    try {
        @include_once $configFile;
        if (defined('DB_HOST') && isset($pdo)) {
            $tables = $pdo->query("SHOW TABLES")->fetchAll(PDO::FETCH_COLUMN);
            if (in_array('users', $tables) && in_array('settings', $tables) && in_array('wishlist_items', $tables) && in_array('rate_limits', $tables)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($db_user) || empty($admin_user) || empty($admin_pass)) {
        ...
        try {
            ...
            // 5. Generate config.php Code from template
            $templateFile = __DIR__ . '/config.template.php';
            if (!file_exists($templateFile)) {
                throw new Exception("Template file (config.template.php) is missing.");
            }
            $configTemplate = file_get_contents($templateFile);
            $configTemplate = str_replace(
                ["'{{DB_HOST}}'", "'{{DB_PORT}}'", "'{{DB_NAME}}'", "'{{DB_USER}}'", "'{{DB_PASS}}'"],
                [var_export($db_host, true), var_export($db_port, true), var_export($db_name, true), var_export($db_user, true), var_export($db_pass, true)],
                $configTemplate
            );
            ...
            if (@file_put_contents($configFile, $configTemplate) !== false) {
                ...
                @file_put_contents($installLock, date('Y-m-d H:i:s') . ' - Installation completed');
            } else {
                $success = true;
            }
